...
(function() {
	...
	var _ = require( 'lodash' ), moment = require( 'moment' ), crypto = require( 'crypto' );
	var praetorian = function( options, tasks ) {
		...
		// tasks are your custom validation routines
		this.tasks = tasks;

		// now fetch the language settings based on the requested language
		this.options.messages = require( './internationalisation/' + this.options.language ); // ISO 639‑1
	...
	praetorian.prototype.validate = function( json, schema, callback ) {
		...
		// grab the options and tasks from the protoype and make them available inside this function
			...
			return {
				...
				hash: function() {
					if( _.isUndefined( check.data.hash ) || ( !_.isString( check.data.hash ) ) ) {
						...
						check.error = options.messages.HASH_INVALID;
					} else if( !_.isObject( check.data.value ) ) {
						// "value" not present or not an object
						check.error = options.messages.HASH_VALUE_NOT_A_OBJECT;
					} else {
						// check the hash "checks out"
						if( crypto.createHmac( 'sha1', options.salt ).update( JSON.stringify( check.data.value )).digest( 'hex' ) !== check.data.hash ) {
							check.error = options.messages.HASH_INVALID;
						}
					}
					return this;
				...
				if( !schema[levelKey] ) {
					...
					if( schema[levelKey].type === primitiveTypes.ARRAY ) {
						...
						if( schema[levelKey].validation ) {
							if( tasks && tasks[schema[levelKey].validation] ) {
								// loop the rules and queue some ancilliary calls
								_.each( tasks[schema[levelKey].validation].rules, function( ruleValue, ruleName ) {
									validator[ruleName]( ruleValue );	
								} );
							} else {
								throw new TypeError( options.messages.PARAMETER_TASKS_NOT_OBJECT );
							}
						}
	...
	praetorian.prototype.requirements = function( schema, callback ) {
		...
		try {
			...
			var addRequirement = function( field ) {
				var requirementRule = options.messages.RULE_NOT_GIVEN;
				// returns the field "requirement" details in a consistent schema
				if( ( field.validation ) && tasks && ( tasks[field.validation].rule ) ) {
					requirementRule = tasks[field.validation].rule;
				} else if( ( field.type ) && ( options.messages['RULE_MUST_BE_' + field.type.toUpperCase()] ) ) {
					requirementRule = options.messages['RULE_MUST_BE_' + field.type.toUpperCase()];
				}
				return {
