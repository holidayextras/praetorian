...
(function() {
	...
	var praetorian = function( options ) {
		if( ( _.isUndefined( options.debug ) ) || ( options.debug !== true ) ) {
			// no console logging, probably don't want to overwrite this locally
			console.log = function(){ };
		} else {
			console.log('[Praetorian is running in debug mode]');
		}
	};
	...
	praetorian.prototype.validate = function( json, schema, callback ) {
		...
		*/
		...
		// loop all schema
		// - if required is set and === true
		// -- check json has a key for that schema key (we're not interested in what the value is or even if it has a value
		// -- at this stage)
		// - else
		// -- not required
		// 
		// - if its of type object (ARRAY or OBJECT) and it has "properties" regardless if its required or not and if there no error
		// --	recurse into the object
		// 
		//
		// TODO: when validating complex types, if the parent is required
		// 
		var requiredJson = function( json, schema, elementPrefix ){
			...
			_.each( schema, function( levelValue, levelKey ) {
				...
				// if schema says type object (ARRAY or OBJECT), schema has "properties" and the json actually has a value...
				if( !_.isUndefined( levelValue.properties ) && !_.isUndefined( json[levelKey] ) ) {
					if( levelValue.type === primitiveTypes.ARRAY && _.isArray( json[levelKey] ) ) {
						_.each( json[levelKey], function( jsonValue, jsonKey ) {
							requiredJson( jsonValue, levelValue.properties, elementPrefix + levelKey + '[' + jsonKey + '].' );
						} );
					} else if( levelValue.type === primitiveTypes.OBJECT && _.isObject( json[levelKey] ) ) {
