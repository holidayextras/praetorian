...
(function() {
	...
	var praetorian = function( options, rules ) {
	...
	praetorian.prototype.validate = function( json, schema, callback ) {
		...
		// grab the options and rules from the protoype and make them available inside this function
				...
				if( !schema[levelKey] ) {
					...
					if( schema[levelKey].type === primitiveTypes.ARRAY ) {
						...
						if( schema[levelKey].validation ) {
							if( rules && rules[schema[levelKey].validation] ) {
								// loop the rules and queue some ancilliary calls
								_.each( rules[schema[levelKey].validation].rules, function( ruleValue, ruleName ) {
									validator[ruleName]( ruleValue );	
								} );
							} else {
								throw new TypeError( options.messages.PARAMETER_RULES_NOT_OBJECT );
							}
						}
	...
	praetorian.prototype.requirements = function( schema, callback ) {
		...
		try {
			...
			var addRequirement = function( field ) {
				...
				if( ( field.validation ) && rules && ( rules[field.validation].rule ) ) {
					requirementRule = rules[field.validation].rule;
