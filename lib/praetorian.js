...
(function() {
	...
	var praetorian = function( myOptions, myTasks ) {
		...
		self.options.messages = require( './internationalisation/' + self.options.language ); // ISO 639‑1
				...
				date: function() {
					...
					if( _.isString( check.data ) ) { 
						...
						if( date.length === 3 ) {
							...
							check.error = self.options.messages.VALUE_FOR_MONTH_INVALID_FORMAT;
						}
					} else {
						// not a string :/
						check.error = self.options.messages.VALUE_FOR_MONTH_INVALID_FORMAT;
					...
					if( schema[levelKey].type === primitiveTypes.ARRAY ) {
						...
						if( schema[levelKey].validation ) {
							if( self.tasks && self.tasks[schema[levelKey].validation] ) {
								...
								_.each( self.tasks[schema[levelKey].validation].rules, function( ruleValue, ruleName ) {
	...
	praetorian.prototype.requirements = function( schema, callback ) {
		...
		try {
			...
			var addRequirement = function( field ) {
				...
				if( ( field.validation ) && self.tasks && ( self.tasks[field.validation].rule ) ) {
