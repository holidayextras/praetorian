...
(function() {
	...
	praetorian.prototype.validate = function( json, schema, callback ) {
		...
		*/
		...
		var parseJson = function( json, schema, schemaType ) {
			...
			_.each( json, function( levelValue, levelKey ) {
				...
				if( _.isUndefined( schema[levelKey] ) ) {
					...
					if( ( schema[levelKey].type === primitiveTypes.ARRAY ) || ( schema[levelKey].type === primitiveTypes.OBJECT ) ) {
						if( schema[levelKey].type === primitiveTypes.ARRAY ) {
							...
							if( _.isArray( json[levelKey] ) ) {
								...
								_.each( json[levelKey], function( arrayElement ) {
									parseJson( arrayElement, schema[levelKey].properties, schema[levelKey].type );
								} );
							} else {
								// ERROR: this value should be an ARRAY but it's not
								// throw up a type error
								console.log( 'frakking object');
							}
						} else {
							if( !_.isArray( json[levelKey] ) ) {
								parseJson( json[levelKey], schema[levelKey].properties, schema[levelKey].type );
							} else {
								// ERROR: this vlaue should be an OBJECT but its an array
								console.log( 'frakking array');
							}
						}
					} else {

						// console.log( 'primitive type [STRING, BOOLEAN etc]' );
						// probably validate some here if we have rules
					}
