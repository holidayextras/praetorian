...
(function() {
	...
	// private variables for our convenience
	var options, tasks;
	...
	var praetorian = function( myOptions, myTasks ) {
		...
		options = _.extend( options, myOptions );

		// now fetch the language settings based on the requested language
		options.messages = require( './internationalisation/' + options.language ); // ISO 639‑1
	...
	praetorian.prototype.generateHash = function( data ) {
		return crypto.createHmac( 'sha1', options.salt ).update( JSON.stringify( data ) ).digest( 'hex' );
	}
			...
			return {
				...
				hash: function() {
					if( _.isUndefined( check.data.hash ) || ( !_.isString( check.data.hash ) ) ) {
						...
						if( praetorian.prototype.generateHash( check.data.value ) !== check.data.hash ) {
							check.error = options.messages.HASH_INVALID;
						}
					}
					return this;
