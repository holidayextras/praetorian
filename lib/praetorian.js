		...
		*/
		...
		var rules = function( data ) {
			...
			return {
				...
				string: function() {
					// check that it is of type "string" and that it has data
					if( !_.isString( check.data ) || check.data.length < 1 ) {
			...
			_.each( schema, function( levelValue, levelKey ) {
				...
				if( json[levelKey] ) {
					...
					if( !levelValue.type ) {
						...
						handleComplexData( _.clone( json[levelKey] ), levelValue.properties );
					}
				}
